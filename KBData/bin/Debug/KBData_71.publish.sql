...
BEGIN
	...
	SELECT o.Id, o.ProductName, Images = STUFF((
		SELECT ';' + pim.ProductImageName
		FROM ProductImage AS pim
		INNER JOIN Product AS p
		ON pim.ProductId = p.Id
		WHERE pim.ProductId = o.Id
		FOR XML PATH, TYPE).value(N'.[1]', N'varchar(max)', N'varchar(max)'), 1, 2, 3, '')
	FROM Product AS o
	GROUP BY o.Id;
