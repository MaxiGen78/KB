...
BEGIN
	...
	SELECT p.Id, p.ProductName, p.ProductPrice, p.ProductQuantity, p.ProductDescription ,  ProductImageList = STUFF((
		SELECT ',' + pim.ProductImageName FROM ProductImage AS pim
		WHERE pim.ProductId = p.Id
		FOR XML PATH('')),1,1,'')
	FROM Product AS p
	GROUP BY p.Id, p.ProductName, p.ProductPrice, p.ProductQuantity, p.ProductDescription;
